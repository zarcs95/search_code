...
class QueueRA(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key = lambda c: c.path_cost)
       # self.A.sort(cmp=lambda a, b: -1 if a.path_cost < b.path_cost else 1 if a.path_cost > b.path_cost else 0)
...
class QueueRAS(Queue):
    ...
    def extend(self, items):
        self.A.extend(items)
        self.A.sort(key = lambda c: c.path_cost + self.problem.h(c))
